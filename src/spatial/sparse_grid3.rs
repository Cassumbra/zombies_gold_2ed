...
//!
//! Elements can be inserted and accessed via their 1d index or 3d index, or
...
use crate::{grid3::Side, point::{GridPoint, Size3d}};
...
impl<T: Clone> SparseGrid3<T> {
    ...
    /// A 3d iterator over all elements in the grid.
    ///
    /// Yields `(IVec3,&mut T)` where `IVec3` is the 3d position of the element in the grid.
    #[inline]
    pub fn iter_3d(&self) -> impl Iterator<Item = (IVec3, &T)> {
        ...
        self.data.iter().map(move |(i, v)| {
            ...
            let y = i / w;
            let z = i / (w * h);
            (IVec3::new(x as i32, y as i32, z as i32), v)
        })
    }

    /// A mutable iterator over all elements in the grid.
    ///
    /// Yields `(IVec,&mut T)` where `IVec3` is the 3d position of the element in the grid.
    #[inline]
    pub fn iter_mut_3d(&mut self) -> impl Iterator<Item = (IVec3, &mut T)> {
        let w = self.width();
        let h = self.height();
    ...
    /// Converts a 3d grid position to it's corresponding 1D index.
    #[inline(always)]
    pub fn transform_lti(&self, pos: impl GridPoint) -> usize {
        let [x, y, z] = pos.as_i_array();
        (z * self.width() as i32 * self.height() as i32 + y * self.width() as i32 + x) as usize
    }
    ...
    pub fn in_bounds(&self, pos: impl GridPoint) -> bool {
        let xy = pos.as_ivec3();
        xy.cmpge(IVec3::ZERO).all() && xy.cmplt(self.size).all()
    }
    ...
    /// Retrieve a value in the grid from it's 3d position.
    ///
    ...
    pub fn get(&self, pos: impl GridPoint) -> Option<&T> {
        ...
        let i = self.transform_lti(pos.as_ivec3());
        self.data.get(&i)
    }

    /// Retrieve a mutable value in the grid from it's 3d position.
    ///
...
mod test {
    ...
    fn insert_row() {
        ...
        let str: String = grid.iter_3d().map(|(_, v)| v).collect();

        assert_eq!("Hello", str);

        for (i, (p, _)) in grid.iter_3d().enumerate() {
    ...
    fn insert_row_at() {
        ...
        let kvp: Vec<_> = grid.iter_3d().collect();

        assert_eq!((IVec3::new(3, 3), &'H'), kvp[0]);
        assert_eq!((IVec3::new(4, 3), &'e'), kvp[1]);
        assert_eq!((IVec3::new(5, 3), &'l'), kvp[2]);
        assert_eq!((IVec3::new(6, 3), &'l'), kvp[3]);
        assert_eq!((IVec3::new(7, 3), &'o'), kvp[4]);
    }
    ...
    fn insert_col_at() {
        ...
        assert_eq!((IVec3::new(3, 3), &'H'), kvp[0]);
        assert_eq!((IVec3::new(3, 4), &'e'), kvp[1]);
        assert_eq!((IVec3::new(3, 5), &'l'), kvp[2]);
        assert_eq!((IVec3::new(3, 6), &'l'), kvp[3]);
        assert_eq!((IVec3::new(3, 7), &'o'), kvp[4]);
    }
